...
import { votingLimiter, receiptLimiter, codeLookupLimiter } from '../middleware/rateLimiter';
...
router.post('/register', authenticate, authorize('student'),
  validate(z.object({ electionCode: z.string().min(1) })), registerForElection);

router.post('/', authenticate, authorize('super_admin'), validate(createElectionSchema), createElection);
router.get('/',  authenticate, authorize('super_admin', 'officer'), listElections);
...
router.post('/:id/members',              authenticate, authorizeElection, uploadCsv, uploadMembers);
router.get('/:id/members',               authenticate, authorizeElection, listMembers);
router.delete('/:id/members',            authenticate, authorize('super_admin'), clearMembers);
router.get('/:id/members/:memberId',     authenticate, authorizeElection, getMember);
router.patch('/:id/members/:memberId',   authenticate, authorizeElection, updateMember);
router.delete('/:id/members/:memberId',  authenticate, authorizeElection, deleteMember);

router.post('/:id/offices',              authenticate, authorizeElection, validate(officeSchema), createOffice);
router.get('/:id/offices',               authenticate, authorizeElection, listOffices);
router.patch('/:id/offices/:officeId',   authenticate, authorizeElection, updateOffice);
...
router.get('/:id/results',         authenticate, authorize('super_admin', 'student'), getResults);
router.post('/:id/publish-results', authenticate, authorizeElection, publishResults);

router.get('/:id/analytics',               authenticate, authorizeElection, getAnalytics);
router.get('/:id/integrity-check',         authenticate, authorize('super_admin'), integrityCheck);
router.get('/:id/integrity-result/:jobId', authenticate, authorize('super_admin'), getIntegrityResult);

router.get('/:id/export/csv', authenticate, authorize('super_admin', 'officer'), exportResultsCsv);
router.get('/:id/export/pdf', authenticate, authorize('super_admin', 'officer'), exportResultsPdf);
